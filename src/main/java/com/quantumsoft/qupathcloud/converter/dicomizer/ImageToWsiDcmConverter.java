...
public class ImageToWsiDcmConverter {
  ...
  public void convertImageToWsiDcm(String imageName) throws QuPathCloudException {
    try {
      Dicomizer.Options options = new Dicomizer.Options()
          .inputPath(pathToImage.toString())
          .outputFolder(pathToFolder.toString())
          .imageName(imageName);
  ...
  private void configurePyramidParameters(Dicomizer.Options options) {
    try (OpenSlide os = new OpenSlide(pathToImage.toFile())) {
      ...
      LOGGER.info(
          "topWidth: " + topWidth + ", topHeight: " + topHeight + ", levelCount: " + levelCount);

      Map<String, String> properties = os.getProperties();
      int tileWidth = (int) readNumericPropertyOrDefault(properties,
          "openslide.level[0].tile-width", -1);
      int tileHeight = (int) readNumericPropertyOrDefault(properties,
          "openslide.level[0].tile-height", -1);
      LOGGER.info("tileWidth: " + tileWidth + ", tileHeight: " + tileHeight);

      boolean reTile = !(tileWidth >= MIN_TILE_SIZE && tileWidth <= MAX_TILE_SIZE
          && tileHeight >= MIN_TILE_SIZE && tileHeight <= MAX_TILE_SIZE);
      ...
      if (rePyramid) {
        int levelsWidthWise = (int) Math
            .ceil(Math.log((double) totalWidth / MAX_TOP_SIZE) / Math.log(2.0));
        int levelsHeightWise = (int) Math
            .ceil(Math.log((double) totalHeight / MAX_TOP_SIZE) / Math.log(2.0));
  ...
  private double readNumericPropertyOrDefault(Map<String, String> properties, String name,
      double defaultValue) {
