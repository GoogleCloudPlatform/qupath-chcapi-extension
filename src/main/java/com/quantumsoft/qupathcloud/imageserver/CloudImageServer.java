...
public class CloudImageServer extends AbstractImageServer<BufferedImage> {
    ...
    private static final boolean DRAW_PLACEHOLDER_TILES = Boolean.getBoolean(DRAW_PLACEHOLDER_TILES_PROPERTY);

    public CloudImageServer(String path, CloudDAO cloudDAO, DicomStore dicomStore) throws QuPathCloudException {
        ...
        originalMetadata = new ImageServerMetadata.Builder(path, pyramid.getWidth(), pyramid.getHeight()).
                setSizeC(3). //channel count
                setPreferredTileSize(pyramid.getTileWidth(), pyramid.getTileHeight()).
                build();
    }
    ...
    public BufferedImage readBufferedImage(RegionRequest request) {
        ...
        for (int x = 0; x <  widthInTiles; x ++) {
            for (int y = 0; y < heightInTiles; y ++) {
                ...
                if (frame != null) {
                    tileCallables.add(new Callable<Void>() {
                        @Override
            ...
            for (int y = 0; y < heightInTiles; y ++) {
                ...
                if (tileImage != null)
                    ...
                    if(DRAW_PLACEHOLDER_TILES) {
                        ...
                        drawPlaceholderTile(g1, request.getX(), request.getY(), graphicsX, graphicsY, downsample);
                    }
                    else {
                ...
                if(DRAW_DEBUG_INFO) {
                    ...
                    g1.drawString(tileX + "/" + tileY + "/" + level, 10 + graphicsX, graphicsY + pyramid.getTileHeight() - 20);

                    g1.setColor(Color.black);
                    g1.drawRect(graphicsX, graphicsY, pyramid.getTileWidth() - 1, pyramid.getTileHeight() - 1);
                }
            }
        }
    ...
    private void drawPlaceholderTile(Graphics g1, int requestX, int requestY, int x, int y, double downsample) {
